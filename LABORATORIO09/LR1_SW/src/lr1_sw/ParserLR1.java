...
public class ParserLR1 {
    ...
    public List<String[]> analizar(List<String> tokens) {
        ...
        while (true) {
            ...
            if (accion.tipo.equals("shift")) {
                ...
                Produccion p = producciones.get(accion.valor);
    ...
    private void cargarTablaAcciones() {
        ...
        actionTable.computeIfAbsent(2, k -> new HashMap<>()).put("$", new Accion("reduce", 4)); // r5 (índice 4 en la lista de producciones)

        // Estado 3
        actionTable.computeIfAbsent(3, k -> new HashMap<>()).put("(", new Accion("shift", 6));

        // Estado 4
        actionTable.computeIfAbsent(4, k -> new HashMap<>()).put("$", new Accion("reduce", 1)); // r2 (índice 1)

        // Estado 5
        actionTable.computeIfAbsent(5, k -> new HashMap<>()).put("switch", new Accion("shift", 3));
        actionTable.computeIfAbsent(5, k -> new HashMap<>()).put("$", new Accion("reduce", 4)); // r5

        // Estado 6
        actionTable.computeIfAbsent(6, k -> new HashMap<>()).put("id", new Accion("shift", 8));

        // Estado 7
        actionTable.computeIfAbsent(7, k -> new HashMap<>()).put("$", new Accion("reduce", 3)); // r4 (índice 3)
        ...
        // Estado 10
        actionTable.computeIfAbsent(10, k -> new HashMap<>()).put("}", new Accion("reduce", 6)); // r7 (índice 6)
        actionTable.computeIfAbsent(10, k -> new HashMap<>()).put("case", new Accion("shift", 13));
        actionTable.computeIfAbsent(10, k -> new HashMap<>()).put("default", new Accion("reduce", 6)); // r7

        // Estado 11
        actionTable.computeIfAbsent(11, k -> new HashMap<>()).put("}", new Accion("reduce", 9)); // r10 (índice 9)
        actionTable.computeIfAbsent(11, k -> new HashMap<>()).put("default", new Accion("shift", 15));

        // Estado 12
        actionTable.computeIfAbsent(12, k -> new HashMap<>()).put("}", new Accion("reduce", 6)); // r7
        actionTable.computeIfAbsent(12, k -> new HashMap<>()).put("case", new Accion("shift", 13));
        actionTable.computeIfAbsent(12, k -> new HashMap<>()).put("default", new Accion("reduce", 6)); // r7
        ...
        // Estado 16
        actionTable.computeIfAbsent(16, k -> new HashMap<>()).put("}", new Accion("reduce", 5)); // r6 (índice 5)
        actionTable.computeIfAbsent(16, k -> new HashMap<>()).put("default", new Accion("reduce", 5)); // r6
        ...
        actionTable.computeIfAbsent(18, k -> new HashMap<>()).put(":", new Accion("reduce", 14)); // r15 (índice 14)

        // Estado 19
        actionTable.computeIfAbsent(19, k -> new HashMap<>()).put("switch", new Accion("reduce", 2)); // r3 (índice 2)
        actionTable.computeIfAbsent(19, k -> new HashMap<>()).put("$", new Accion("reduce", 2)); // r3

        // Estado 20
        actionTable.computeIfAbsent(20, k -> new HashMap<>()).put("switch", new Accion("shift", 26));
        actionTable.computeIfAbsent(20, k -> new HashMap<>()).put("}", new Accion("reduce", 4)); // r5

        // Estado 21
        actionTable.computeIfAbsent(21, k -> new HashMap<>()).put("switch", new Accion("shift", 29));
        actionTable.computeIfAbsent(21, k -> new HashMap<>()).put("}", new Accion("reduce", 4)); // r5
        actionTable.computeIfAbsent(21, k -> new HashMap<>()).put("break", new Accion("reduce", 4)); // r5
        actionTable.computeIfAbsent(21, k -> new HashMap<>()).put("default", new Accion("reduce", 4)); // r5
        actionTable.computeIfAbsent(21, k -> new HashMap<>()).put("case", new Accion("reduce", 4)); // r5

        // Estado 22
        actionTable.computeIfAbsent(22, k -> new HashMap<>()).put(":", new Accion("reduce", 12)); // r13 (índice 12)

        // Estado 23
        actionTable.computeIfAbsent(23, k -> new HashMap<>()).put("num", new Accion("shift", 30));

        // Estado 24
        actionTable.computeIfAbsent(24, k -> new HashMap<>()).put("}", new Accion("reduce", 8)); // r9 (índice 8)

        // Estado 25
        actionTable.computeIfAbsent(25, k -> new HashMap<>()).put("}", new Accion("reduce", 4)); // r5
        ...
        // Estado 27
        actionTable.computeIfAbsent(27, k -> new HashMap<>()).put("}", new Accion("reduce", 11)); // r12 (índice 11)
        actionTable.computeIfAbsent(27, k -> new HashMap<>()).put("case", new Accion("reduce", 11)); // r12
        actionTable.computeIfAbsent(27, k -> new HashMap<>()).put("break", new Accion("shift", 34));
        actionTable.computeIfAbsent(27, k -> new HashMap<>()).put("default", new Accion("reduce", 11)); // r12

        // Estado 28
        actionTable.computeIfAbsent(28, k -> new HashMap<>()).put("switch", new Accion("shift", 29));
        actionTable.computeIfAbsent(28, k -> new HashMap<>()).put("}", new Accion("reduce", 4)); // r5
        actionTable.computeIfAbsent(28, k -> new HashMap<>()).put("case", new Accion("reduce", 4)); // r5
        actionTable.computeIfAbsent(28, k -> new HashMap<>()).put("break", new Accion("reduce", 4)); // r5
        actionTable.computeIfAbsent(28, k -> new HashMap<>()).put("default", new Accion("reduce", 4)); // r5

        // Estado 29
        actionTable.computeIfAbsent(29, k -> new HashMap<>()).put("(", new Accion("shift", 36));

        // Estado 30
        actionTable.computeIfAbsent(30, k -> new HashMap<>()).put(":", new Accion("reduce", 13)); // r14 (índice 13)

        // Estado 31
        actionTable.computeIfAbsent(31, k -> new HashMap<>()).put("}", new Accion("reduce", 3)); // r4

        // Estado 32
        actionTable.computeIfAbsent(32, k -> new HashMap<>()).put("id", new Accion("shift", 37));

        // Estado 33
        actionTable.computeIfAbsent(33, k -> new HashMap<>()).put("}", new Accion("reduce", 7)); // r8 (índice 7)
        actionTable.computeIfAbsent(33, k -> new HashMap<>()).put("case", new Accion("reduce", 7)); // r8
        actionTable.computeIfAbsent(33, k -> new HashMap<>()).put("default", new Accion("reduce", 7)); // r8

        // Estado 34
        actionTable.computeIfAbsent(34, k -> new HashMap<>()).put(";", new Accion("shift", 38));

        // Estado 35
        actionTable.computeIfAbsent(35, k -> new HashMap<>()).put("}", new Accion("reduce", 3)); // r4
        actionTable.computeIfAbsent(35, k -> new HashMap<>()).put("case", new Accion("reduce", 3)); // r4
        actionTable.computeIfAbsent(35, k -> new HashMap<>()).put("break", new Accion("reduce", 3)); // r4
        actionTable.computeIfAbsent(35, k -> new HashMap<>()).put("default", new Accion("reduce", 3)); // r4
        ...
        // Estado 38
        actionTable.computeIfAbsent(38, k -> new HashMap<>()).put("}", new Accion("reduce", 10)); // r11 (índice 10)
        actionTable.computeIfAbsent(38, k -> new HashMap<>()).put("case", new Accion("reduce", 10)); // r11
        actionTable.computeIfAbsent(38, k -> new HashMap<>()).put("default", new Accion("reduce", 10)); // r11
        ...
        // Estado 42
        actionTable.computeIfAbsent(42, k -> new HashMap<>()).put("}", new Accion("reduce", 6)); // r7
        actionTable.computeIfAbsent(42, k -> new HashMap<>()).put("case", new Accion("shift", 13));
        actionTable.computeIfAbsent(42, k -> new HashMap<>()).put("default", new Accion("reduce", 6)); // r7

        // Estado 43
        actionTable.computeIfAbsent(43, k -> new HashMap<>()).put("}", new Accion("reduce", 6)); // r7
        actionTable.computeIfAbsent(43, k -> new HashMap<>()).put("case", new Accion("shift", 13));
        actionTable.computeIfAbsent(43, k -> new HashMap<>()).put("default", new Accion("reduce", 6)); // r7
    }

    private void cargarTablaGoto() {
        // Estado 0
        gotoTable.computeIfAbsent(0, k -> new HashMap<>()).put("S", 1);
        gotoTable.get(0).put("SWITCH", 2);

        // Estado 2
        gotoTable.computeIfAbsent(2, k -> new HashMap<>()).put("SWITCH", 5);
        gotoTable.get(2).put("MASSWITCH", 4);

        // Estado 5
        gotoTable.computeIfAbsent(5, k -> new HashMap<>()).put("SWITCH", 5);
        gotoTable.get(5).put("MASSWITCH", 7);

        // Estado 10
        gotoTable.computeIfAbsent(10, k -> new HashMap<>()).put("CASES", 12);
        gotoTable.get(10).put("MASCASES", 11);
        ...
        gotoTable.get(12).put("MASCASES", 16);
        ...
        // Estado 20
        gotoTable.computeIfAbsent(20, k -> new HashMap<>()).put("SWITCH", 26);
        gotoTable.get(20).put("MASSWITCH", 24);

        // Estado 21
        gotoTable.computeIfAbsent(21, k -> new HashMap<>()).put("SWITCH", 29);
        gotoTable.get(21).put("MASSWITCH", 27);

        // Estado 25
        gotoTable.computeIfAbsent(25, k -> new HashMap<>()).put("SWITCH", 26);
        gotoTable.get(25).put("MASSWITCH", 31);

        // Estado 27
        gotoTable.computeIfAbsent(27, k -> new HashMap<>()).put("BREAK", 33);

        // Estado 28
        gotoTable.computeIfAbsent(28, k -> new HashMap<>()).put("SWITCH", 29);
        gotoTable.get(28).put("MASSWITCH", 35);

        // Estado 42
        gotoTable.computeIfAbsent(42, k -> new HashMap<>()).put("CASES", 12);
        gotoTable.get(42).put("MASCASES", 44);

        // Estado 43
        gotoTable.computeIfAbsent(43, k -> new HashMap<>()).put("CASES", 12);
        gotoTable.get(43).put("MASCASES", 45);
